...
Following are the available samples, *only one can be enabled at a time* <br>
        * DEMO_PING<br>
        The device will ping an external machine (define the external device IP in the same header file) <br>
        * DEMO_TCP<br>
        A sample TCP server, use port 9640 (can be changed in the header file), server will acknowledge the messages received <br>
        * DEMO_UDP<br>
